...
export function EquipeModal({ open, onClose, editingEquipe }: Props) {
  ...
  const inputCls = 'w-full bg-slate-900 text-white text-sm px-2.5 py-2 rounded-lg border border-slate-600 outline-none focus:border-blue-500 transition-colors';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-5 space-y-5">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {COLORS.map((c) => (
                  <button
                    key={c}
                    onClick={() => setCor(c)}
                    className={`w-7 h-7 rounded-lg transition-transform ${cor === c ? 'ring-2 ring-white scale-110' : 'hover:scale-105'}`}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-semibold text-white">Funções / Pessoal</label>
              <button
                onClick={addFuncao}
            ...
            <div className="rounded-xl border border-slate-700 overflow-hidden">
              <table className="w-full">
                ...
                <tbody>
                  {funcoes.length === 0 && (
                    <tr>
                      <td colSpan={5} className="text-center text-slate-500 text-sm py-4">
                    ...
                    <tr key={i} className="border-t border-slate-700">
                      <td className="px-2 py-1.5">
                        <input
                          ...
                          className="w-16 bg-slate-900 text-white text-sm px-2 py-1.5 rounded border border-slate-600 outline-none focus:border-blue-500"
                        />
                      </td>
                      <td className="px-2 py-1.5">
                        <input
                          ...
                          className="w-full bg-slate-900 text-white text-sm px-2 py-1.5 rounded border border-slate-600 outline-none focus:border-blue-500"
                        />
                      </td>
                      <td className="px-2 py-1.5">
                        <input
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-semibold text-white flex items-center gap-1.5">
                <Wrench size={15} className="text-slate-400" /> Equipamentos
              </label>
              <button
                onClick={addEquip}
                className="flex items-center gap-1 text-xs bg-blue-600 hover:bg-blue-500 text-white px-2.5 py-1.5 rounded-lg transition-colors"
              ...
              <table className="w-full">
                ...
                <tbody>
                  {equipamentos.length === 0 && (
                    <tr>
                      <td colSpan={3} className="text-center text-slate-500 text-sm py-4">
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-700 sticky bottom-0 bg-slate-800 rounded-b-2xl">
          <button
            onClick={onClose}
