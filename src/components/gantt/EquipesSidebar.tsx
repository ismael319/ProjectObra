...
export function EquipesSidebar() {
  ...
  return (
    <div className="w-52 bg-slate-900 border-r border-slate-700 flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700">
        <h2 className="text-sm font-semibold text-white tracking-wide uppercase">Equipes</h2>
        <button
          onClick={openNew}
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-2.5 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white truncate">{eq.nome}</p>
                    {eq.funcoes.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {eq.funcoes.map((f, i) => (
                          <span
                            key={i}
                            className="text-xs px-2 py-0.5 rounded-full bg-blue-900/60 text-blue-300 border border-blue-800"
                          >
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => openEdit(eq)} className="text-slate-400 hover:text-white p-1">
                    <Pencil size={12} />
                  </button>
                  <button onClick={() => deleteEquipe(eq.id)} className="text-slate-400 hover:text-red-400 p-1">
                    <Trash2 size={12} />
                  </button>
                </div>
              </div>
