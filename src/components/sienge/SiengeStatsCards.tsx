  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {stats.map((stat) => (
        <div
          key={stat.label}
          className="relative bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700/80 shadow-sm overflow-hidden"
        >
          <div
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{stat.valor(itens)}</p>
        </div>
      ))}
    </div>
  )
