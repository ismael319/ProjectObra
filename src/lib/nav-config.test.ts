...
describe('nav-config', () => {
  it('expõe somente os módulos contratados', () => {
    ...
    expect(sections.map((section) => section.title)).toEqual(['Engenharia', 'Qualidade'])
    expect(sections[1].items[0].children?.[0].path).toBe('/dashboard/qualidade/concreto/dashboard')
  })
  ...
  it('usa destino alternativo somente para agrupadores sem página própria', () => {
    ...
    const efetivo = sections[0].items[1]
    const concreto = sections[1].items[0]
  ...
  it('resolve títulos específicos sem tratar rota desconhecida como visão geral', () => {
    ...
    expect(getDashboardRouteTitle('/dashboard/rota-inexistente')).toBe('FGI Decision')
  })
})
